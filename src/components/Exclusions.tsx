...
const Exclusions: React.FC = () => {
  const studies = [
    { name: "Étude de sol G1 et G2", cost: "2 000 à 3 500 €", provider: "Bureau d'études géotechniques agréé" },
  ...
  const insurances = [
    { name: "Assurance Dommage-Ouvrage", cost: "3 000 à 4 500 €", period: "À souscrire avant ouverture du chantier" },
  ];

  const exclusions = [
    "Aménagement de cuisine (uniquement attentes des réseaux)",
    "Aménagement paysager",
    "Clôtures et portails",
    "Raccordements aux réseaux (si éloignés)",
    "Bassin de rétention (si imposé par la mairie)"
  ];
                ...
                <tbody>
                  {studies.map((study, index) => (
                    <tr key={index} className="bg-white">
                      ...
                      <td className="px-4 py-3 text-right font-mono font-semibold" style={{ color: '#c1a16a' }}>{study.cost}</td>
                ...
                <tbody>
                  {insurances.map((insurance, index) => (
                    <tr key={index} className="bg-white">
                      ...
                      <td className="px-4 py-3 text-right font-mono font-semibold" style={{ color: '#c1a16a' }}>{insurance.cost}</td>
