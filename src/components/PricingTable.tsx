...
const PricingRow: React.FC<PricingRowProps> = ({ title, htAmount, tva, ttcAmount, isSubtotal = false }) => {
  ...
  return (
    <tr className={baseClass} style={borderStyle}>
      ...
      <td className="px-4 py-3 text-right text-gray-900 font-mono">{htAmount}</td>
      <td className="px-4 py-3 text-right text-gray-600 font-mono">{tva}</td>
      <td className="px-4 py-3 text-right font-mono font-semibold" style={{ color: '#c1a16a' }}>{ttcAmount}</td>
    </tr>
  );
};
